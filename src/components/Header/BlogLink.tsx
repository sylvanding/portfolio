import { useTranslation } from "../../i18n";

export default function BlogLink() {
  const { t } = useTranslation();

  return (
    <a
      href="https://sylvanding.online/"
      target="_blank"
      rel="noopener noreferrer"
      className="flex-center group hover:shadow-bottom dark:hover:shadow-bottom-inv h-11 gap-2 rounded border-2 border-black px-5 py-2 font-semibold text-black transition-all duration-300 hover:bg-black hover:text-white active:translate-y-1 dark:border-white dark:text-white dark:hover:bg-white dark:hover:text-black"
    >
      <span className="text-sm lg:text-base">{t.nav.blog}</span>
      <svg
        width="16"
        height="16"
        viewBox="0 0 20 20"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        aria-hidden="true"
      >
        <path
          d="M10.8335 9.16671L17.6668 2.33337"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M18.3335 5.66663V1.66663H14.3335"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M9.1665 1.66663H7.49984C3.33317 1.66663 1.6665 3.33329 1.6665 7.49996V12.5C1.6665 16.6666 3.33317 18.3333 7.49984 18.3333H12.4998C16.6665 18.3333 18.3332 16.6666 18.3332 12.5V10.8333"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </a>
  );
}
